...
def predict():
    ...
    image_arr = tf.keras.applications.resnet50.preprocess_input(
        np.expand_dims(np.array(face_img), axis=0)
    )
    pred = model.predict(image_arr)[0][0]
    label = int(pred > 0.5)

    return jsonify({"prediction": float(pred), "label": label, "face_image": face_b64})
